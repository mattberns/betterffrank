...
significance at 0.05 is reachable). Current standing after the 2026-07-25
clean-data rebuild (53-feature model, offset-log vs_adp, drafted-slot VORP
curve, QB8+TE8 streaming replacement, alpha=300/shrink=0.3):
model 0.5237 / ECR 0.5205 / ADP 0.5040 mean spearman_vorp; **beats ADP
+0.0197 (7/8, p_one = 0.0469 — clears 0.05 by a hair, two-sided 0.094 does
NOT); does NOT beat ECR — +0.0032 at 4/8, p_one = 0.227, a coin flip.** The
primary benchmark is ECR, so the correct summary is "beats the market, matches
the experts". Do not restore the older "edges ECR" language; it was measured
on contaminated inputs. **Tune-window mean is 0.4293 vs ADP 0.4305 / ECR
0.4298 — the model has NO edge on its own validation window.** That is NOT a
contradiction with the test result: the ANCHOR's own quality differs by era
(ECR−ADP is −0.0008 on tune, +0.0165 on test, ECR 7/8, widening to +0.0554 by
2025), so of the +0.0197 test win over ADP, **+0.0165 is inherited from ECR
and only +0.0032 is the ridge**. The model's own contribution is roughly
constant across windows (+0.0005 tune, +0.0032 test); what moved is what it
stands on. See REPORT.md §3a — documented there, in README, and in the "Are the
experts actually better than the market?" section of the site's front-page
ARTICLE (index.html; still generated by `_anchor_table` / the `anchor` payload
block in `bff/site.py`, so it never drifts from the artifacts). Integrity ledger: 2018-2020 briefly served as tune folds
...
looks: FOUR on 2026-07-23, SIX on 2026-07-24, THREE on 2026-07-25 (THIRTEEN
total) — count future looks, keep them rare. **Thirteen is already a lot; the
0.0469 ADP p-value should not be read as a clean pre-registered result.** Three 07-24 looks were a-priori data-corrections to
...
spend a look on a tune-window gain under ~0.01. Same-day history (07-23): ECR window
...
- Hyperparameters are tuned ONLY on walk-forward validation seasons
  ...
  (the VORP-tuned winner is alpha = 300, shrink = 0.3 — alpha now sits ON the
  grid maximum, a starved-fit warning sign, not a result). Test
...
| `bff.methodology_audit` | `reports/methodology_audit.json` (9 blocks: outcome split-half reliability 0.894, features→anchor R² 0.655, walk-forward residual r 0.0695 / R² −0.0028, ridge geometry, bust-tail loss share 72.5% from 11.5% of rows, S=8 power incl. MDE +0.0112 and 82 seasons needed vs ECR, permuted-target runs 0.4269/0.4350/0.4206 vs shipped 0.4293 / null 0.4306, clip sweep, per-player estimand). Deterministic (fixed seeds); asserts its local clip-variable fitter reproduces `bff.model` exactly before writing. NEVER a test look |
...
- **`FIRST_TARGET = 2012` — 2011 is OUT and must stay out.** See "Three input
  ...
  ADP board and is void. Restoring 2011 buys +0.0153 of tune score from an
  input known to be wrong. And do NOT shift the whole protocol
  forward to buy a sixth ECR-clean fold: that moves 2018 — the model's only
  ADP loss, already scored twelve times — out of the test set, manufacturing
  a 7-of-7 record by deletion.
