...
stands on. See REPORT.md §3a — documented there, in README, and on the site's
Results page (generated by `_anchor_table` / the `anchor` payload block in
`bff/site.py`, so it never drifts from the artifacts). Integrity ledger: 2018-2020 briefly served as tune folds
...
looks: FOUR on 2026-07-23, SIX on 2026-07-24, TWO on 2026-07-25 (TWELVE
total) — count future looks, keep them rare. **Twelve is already a lot; the
...
wrong. Same-day history (07-23): ECR window
