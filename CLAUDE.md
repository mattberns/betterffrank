...
walk-forward with zero leakage. Protocol (authoritative: `reports/REPORT.md`): tune **2013-2017 (5 folds**,
spearman_vorp, frozen grid), test 2018-2025 (S=8, sign-flip floor 0.0039 —
significance at 0.05 is reachable). Current standing after the 2026-07-27
feature audit (look 15; 53-feature model, offset-log vs_adp, drafted-slot
VORP curve, QB8+TE8 streaming replacement, alpha=300/shrink=0.3 — shrink
moved back down from 0.5 with the audit):
model 0.5245 / ECR 0.5190 / ADP 0.5045 mean spearman_vorp; **beats ADP
+0.0201 (6/8, p_one = 0.0313 — but two-sided 0.0625, so the ADP claim rests
on the one-sided test only; §2f's brief clears-both reading did not survive
the feature audit); does NOT beat ECR — +0.0056 at 5/8,
p_one = 0.117.** The primary benchmark is ECR, so the correct summary is
"beats the market, matches the experts". Do not restore the older "edges ECR"
language; it was measured on contaminated inputs. **Tune-window mean is
0.4354 vs ADP 0.4336 / ECR 0.4356 / anchor-only null 0.4366 — the model has
NO tune-window edge again** (§2f briefly manufactured +0.0026 over the null;
part of that was fit to the corrupted injury variable and truncated career
window that look 15 repaired). The
anchor's own quality differs by era (ECR−ADP is +0.0020 on tune, +0.0145 on
test, ECR 7/8, widening to +0.0548 by 2025), so of the +0.0201 test win over
ADP, **+0.0145 is inherited from ECR and only +0.0056 is the ridge** — the
ridge's own contribution is −0.0002 tune / +0.0056 test, i.e. the test-window
ridge edge is NOT corroborated by the tune window. See REPORT.md §3a — documented there, in README, and in the
"Are the experts actually better than the market?" section of the site's
front-page ARTICLE (index.html; still generated by `_anchor_table` / the
`anchor` payload block in `bff/site.py`, so it never drifts from the
artifacts). Integrity ledger: 2018-2020 briefly served as tune folds
...
on the tune window with no look spent. **Look 11 (07-25): the 2012-2014 ECR
backfill** — see the dedicated section below; it did NOT pass a tune gate
(costs −0.0050) and was shipped anyway as a data completion under the existing
anchor rule. **Look 12 (07-25): the clean-data rebuild** — repaired 2012 ADP
board, draft-year-gated ECR name resolution, and 2011 REMOVED from training
(see "Three input defects" below). Also not gated on the metric; it makes the
metric substantially worse and shipped because the inputs it removes are
wrong. **Look 13 (07-25): grid extension + inj_pos block, REVERTED** — the
grid was extended (alpha ×1000, shrink down to 0.10) jointly with the
`inj_pos` block; tune 0.4293 → 0.4361 (+0.0068, LOFO-validated, the first
tune-window ECR win), but test 0.5237 → 0.5200 (−0.0037), ECR edge → −0.0004,
ADP p_one 0.0469 → 0.0820. Rolled back; the grid stays frozen and the
columns stay as the `inj_pos` CANDIDATE_BLOCK. Lesson (full text in the grid
comment in `bff/model.py`): the 5-fold tune mean has SE ~0.0022, so do not
spend a look on a tune-window gain under ~0.01. **Look 14 (07-27): the
actuals repair** — a data correction
(fabricated 0.0-point outcome seasons for four drafted players),
tune-window-validated first, one test look. **Look 15 (07-27): the
player-season feature audit** (REPORT §2g) — five data corrections bundled
into ONE look: (1) injury features re-derived as Questionable/Doubtful/Out
weeks UNIONED with reserve-list weeks (the old count included the
pre-2016-only "Probable" status, making tune targets measure ~2x what test
targets measured, AND was blind to IR — CMC 2024: 13 games missed, 3 listed
weeks); (2) stale `ctx_team_qb.parquet` rebuilt (it was built from the
BROKEN 2012 ADP board two days after the board was repaired — 2012's four QB
features were degenerate); (3) `build_pool` team fallback to the week-1
roster (17 FA-listed pool rows, incl. Spiller 2013 at ADP 6, had every team
feature zero-filled); (4) 2019 ECR "Mike Davis" WR/RB duplicate fixed by
id-level keep-best-rank dedup (do NOT position-gate the name fallback — it
legitimately rescues cross-position listings); (5) trajectory career history
extended to source seasons 1999-2025 (was 2010+, truncating tune-fold
careers to 2-7 seasons vs 8-15 in test). Tune 0.4392 → 0.4354, hyperparams
back to (300, 0.3), one test look → the current standing above. LOFO on the
repaired surface (recorded, NOT acted on — both sub-SE): inj trio −0.0016,
trajectory +0.0018. Audit false-positives worth remembering: Guerendo's
0.0-point 2025 is REAL (special-teams only, pbp-confirmed), Shaheed's 18
games is REAL (trade dodged both byes), `opp_fp_oe_pg`'s 2019 level shift is
real scoring-era drift over deliberately fixed weights.
Same-day history (07-23): ECR window
widened to 2015-2025 via Wayback backfill (`bff/ecr_wayback.py`), extended
again to 2012-2025 on 07-25; feature
...
## Three input defects fixed 2026-07-25 — read before quoting any old number

Chasing the ECR gap turned up three bad inputs. All are fixed; together they
are why every headline number moved and why the ECR claim is gone.

1. **ECR started in 2015**, so tune folds 2012-2014 anchored on ADP while all
   test folds anchor on ECR. Backfilled 2012-2014 from Wayback. See the
   dedicated section below.
2. **The 2012 ADP board was half missing and looked fine.** FFC's API returns
   93 players stopping at ADP 156.9 with ten >4-slot gaps; the 2012-09-02
   capture has 189 to ADP 172.7. The API also reports total_drafts 303 while
   capping times_drafted at 130 (the capture has Foster at 251), so its stored
   2012 PPR data was pruned. 2012 is a SCORED fold that was being graded on 91
   pool players against ~150 elsewhere. Fixed via `LEGACY_RECOVERY` in
   `bff/adp.py` (cached `ppr_2012_wayback.json`, 150-player floor assert).
   2012 now has 162 skill players, 98.7% top-150 gsis. **Never revert 2012 to
   the API.**
3. **`bff/ecr.py` had no draft-year gating** on its name fallbacks, so
   "Frank Gore" (2005) collided with "Frank Gore Jr." (2024) after suffix
   stripping and a top-40 RB dropped out of 2012 AND 2013. Now gated on
   `draft_year <= season` (what `bff/adp.py` always did), plus aliases
   (Steve→Stevie Johnson, Christopher→Chris Ivory, Chris "Beanie"→Beanie
   Wells) and `norm_name` strips double quotes. Verified strictly additive:
   0 ids changed, 0 lost, 8 filled. Pool ECR coverage 100% (2012) / 99.3%
   (2013; the one miss, Blount at pool rank 148, is truly absent from the
   Aug-18 board).

**And 2011 was REMOVED from training** (`FIRST_TARGET` 2011 → 2012). It has no
ECR and never will (no cheatsheet capture before 2012-06-10), so its 160 rows
anchored on ADP with `vs_adp = 0`. The old defence — "a training row only
supplies (market rank, outcome)" — is wrong for this architecture: the ridge
learns a RESIDUAL RELATIVE TO ITS ANCHOR, so 2011 taught ADP-correction that
then gets applied to an ECR anchor, and `vs_adp = 0` asserts exact
expert/market agreement for 160 players with no expert board. Cost, reported
not hidden: fold 2012 loses its training window (tune → 5 folds), the tuner
pins alpha at the grid maximum 300, and the tune-window edge over both
baselines vanishes. **Restoring 2011 would raise the tune mean to 0.4446 and
manufacture a +0.0140 tune-window ADP edge out of an input we know is wrong.
Do not do it.** A metric that improves when fabricated rows are added is
evidence the edge rested on them, not evidence they belong.

Note a concurrent session had previously written the OPPOSITE conclusion into
`bff/model.py` and this file (rejecting the 2012 shift on a −0.0047 tune
delta). That analysis used the BROKEN 2012 board; it is superseded.

## The fourth input defect: actuals silently dropped drafted players (fixed 2026-07-27)

`bff/actuals.py` filtered weekly rows on `position ∈ {QB,RB,WR,TE}`, and the
nflverse position field is null for some ids and uses roster positions the
fantasy boards don't. Four board players therefore had real seasons recorded
as **0.0 points**: Trent Richardson 2012/2013/2014 (null position; 254.7 /
144.9 / 117.8 real PPR — his 2013 row was ADP **10** coded as a zero-point
rookie), Mike Tolbert 2012 and Marcel Reece 2013 (raw position FB), and
Travis Hunter 2025 (raw position CB, two-way player). Three damage channels:
fabricated training targets, corrupted prev-season features
(`has_prior = 0`), and corrupted BACKTEST GROUND TRUTH (a pool player missing
from actuals scores as 0 — folds 2013, 2014 and test 2025 graded every board
against a false outcome). This is why tune fold 2015 (training window
2012-2014 = all three TR seasons) responded so strongly to bust-capping and
bust-dropping experiments; the §2f story in REPORT.md has the full audit.
Fix: rows whose raw position fails the filter fall back to the player's
ADP/ECR board position (`board_positions`), and `check_board_coverage`
ASSERTS every ADP-board player with nonzero raw weekly points appears in
actuals — investigate a failure, never weaken it. Effects: tune 0.4293 →
0.4392 (ADP 0.4336 / ECR 0.4356 — all three rise, the model most), tuner
moves shrink 0.3 → 0.5, test 0.5237 → 0.5250 with the ADP claim briefly
clearing two-sided 0.05 (look 14; both reversed by the look-15 feature
audit — see the standing paragraph). Numbers quoted from any earlier state (incl. the
"Restoring 2011" figures below and every §4 rejected-block delta) were
measured against the corrupted truth. The bust-tail/loss-mismatch findings in
`reports/methodology_audit.json` survive but shrank (72.5% → 70.0% of squared
error from the sub-50-point tail); the residual-cap and drop-injury-rows
ideas were mostly deleting these fabricated rows — on corrected data they are
worth ~0 at fixed hyperparameters. Do not re-litigate them without new
evidence.

## The tune window was half-ADP-anchored until 2026-07-25

Until that date `ecr.parquet` started in 2015, so folds 2012-2014 anchored on
`log(adp_rank)` while every test fold anchors on `log(ecr_rank)`, and `vs_adp`
was learnable in only 2 of 6 folds. **The tuning surface was grading against
the easy benchmark on half its folds.** `bff/ecr_wayback.py` now backfills
2012-2014 from three more Wayback cheatsheet captures (2012-09-01,
2013-08-18 — the last capture archived that preseason — and 2014-09-01).

- Cost: tune mean 0.4617 → **0.4567** (−0.0050, negative in 5 of 6 folds).
  Decomposed against a no-`vs_adp` run, `vs_adp`'s own contribution is
  UNCHANGED (+0.0018 → +0.0020); the whole loss is the anchor swap. The new
  boards are not bad data — as a baseline they beat ADP on the tune window by
  +0.0088, the same edge ECR shows in 2015-2017.
- Benefit, and the reason it shipped without passing a gate: the tune-window
  edge over ECR was measurable on only 3 folds and read **+0.0220** there,
...
- Anchor: `mkt_val = log(ecr_rank)` when a preseason ECR snapshot exists for
  the season (2012-2025 + 2026), else `log(adp_rank)`; then ordinal re-rank per
...
- `vs_adp` (regression feature) = `log((ecr_rank + 10) / (adp_rank + 10))`,
  exactly 0.0 when ECR is missing (only the 2011 training rows now). **Sign
...
- **2026 IS NOT AVAILABLE, and that is a market fact, not a fetch bug** (checked
  ...
  gate is scored on the tune window and does not involve 2026 at all, so a posted
...
- Hyperparameters are tuned ONLY on walk-forward validation seasons
  2013-2017, on the frozen grid alpha ∈ {3, 10, 30, 100, 300} × shrink ∈
  {0.3, 0.5, 0.7, 1.0}, scoring mean `spearman_vorp`. Do not expand the grid
  (the VORP-tuned winner is alpha = 300, shrink = 0.3 as of the 2026-07-27
  feature audit — alpha still sits ON the grid maximum, a starved-fit
  warning sign, not a result). Test
  seasons 2018-2025 are never touched for tuning or feature selection.
- Preseason ECR in `data/processed/ecr.parquet`: 2012-2020 from Wayback
  ...
  `data/raw/wayback_ppr/`; overall displayed rank = the `ecr` value. 2014-2020
  rows carry a real `fantasypros_id` → 100% top-150 gsis match; **2012-2013
  predate FantasyPros' inline ids** and resolve by name, giving 96-97%
  top-150 / 98.9% and 97.3% POOL coverage — the misses are crosswalk-ambiguous
  names the ADP board cannot resolve either, so they are not in the pool),
  2021-2025
...
```
# one-time: backfill 2012-2020 ECR from Wayback (needs network; caches HTML,
# then reruns are offline). Produces data/raw/db_fpecr_wayback.parquet.
uv run python -m bff.ecr_wayback

# data rebuild (only if raw data changes). ORDER MATTERS: bff.context_data
# reads adp.parquet (expected_qb per team), so any ADP change REQUIRES a
# context_data + context_features rerun — skipping it is how the repaired
# 2012 board served a stale ctx_team_qb for two days (look 15, defect 2).
...
# candidate-feature selection (tune window 2013-2017 ONLY; never the test set)
...
# player-level audit dump of the model's own inputs (read-only view, not scored)
uv run python -m bff.audit [--season 2026]       # -> reports/audit_2026{,_columns}.csv

# methodology audit — power, permuted-target noise floor, redundancy vs the
# anchor, loss/metric mismatch. Tune window + published scores ONLY, no test
# look. Cached artifact; the site's front-page article is built from it.
...
| `bff.audit [--season Y]` | `reports/audit_Y.csv` (one row per board player × 174 cols: identity, market/anchor, output, the 53 in-model features, per-player `c_*` contributions, then every internal/inert candidate column) + `reports/audit_Y_columns.csv` (column dictionary with group / in_model / fitted ridge coef). Re-runs `build_dataset` + `tune` + `fit_predict`, so `our_rank`/`pred_vorp` match `rankings_Y.csv` exactly; writes nothing under `data/` and is never a test look |
| `bff.methodology_audit` | `reports/methodology_audit.json` (9 blocks, post-look-15 values: outcome split-half reliability 0.894, features→anchor R² 0.664, walk-forward residual r 0.0082 / R² −0.0087, ridge geometry, bust-tail loss share 70.0% from 10.9% of rows, S=8 power incl. MDE +0.0116 and 30 seasons needed vs ECR, permuted-target runs 0.4260/0.4263/0.4220 vs shipped 0.4354 / null 0.4366, clip sweep, per-player estimand). Deterministic (fixed seeds); asserts its local clip-variable fitter reproduces `bff.model` exactly before writing. NEVER a test look |
...
4. `/docs` static site — regenerate with `uv run python -m bff.site`
   ...
   **The front page is an ARTICLE, not a dashboard** ("It's really hard to
   beat the ECR"), and its numbers come from `reports/methodology_audit.json`
   as well as from preds/scores — so a results change also needs
   `uv run python -m bff.methodology_audit` re-run BEFORE `bff.site`, or the
   article silently keeps quoting the old audit. `bff.site` errors out if the
   JSON is missing, but it cannot tell that a present one is stale.
...
- Never tune or feature-select on the test seasons 2018-2025; the 2013-2017
...
- **VONA (`bff/vona.py`) is a draft-strategy OVERLAY, never part of the scored
  ...
  waiting per pick (`reports/vona_2026.csv` + stdout; the site's Draft page
  is the draftsim simulator, which computes its own client-side VONA from
  the same board). It must NOT feed `REPL_RANKS`, the
...
- Three different `norm_name` variants exist across adp/ecr/crosswalk joins;
  do not unify them.
- **Injury features count ONLY Questionable/Doubtful/Out weeks, unioned with
  reserve-list (RES/PUP) roster weeks** (`QDO` / `RESERVE_STATUS` in
  `bff/situation_features.py`). Do not "simplify" to `report_status
  non-null` (Probable existed 2010-2015 only → 2x tune/test regime shift) and
  do not add INA (2020+-only, includes healthy scratches). See the BLOCK 2
  docstring.
- `bff.ecr` asserts (season, gsis_id) uniqueness and dedupes id collisions by
  keeping the best rank. Do NOT position-gate the name-only fallback instead;
  it legitimately rescues cross-position listings (Patterson, Jordan
  Matthews).
- `context_features.build_pool` falls back to the week-1 roster team when the
  ADP board says FA/null; `team_missing` means BOTH sources failed (0 rows
  everywhere since the fix).
- Trajectory career history reads raw `player_stats_{1999..2009}.parquet`
  via `legacy_career_ppg()` in addition to actuals. Do NOT extend
  `bff/actuals.py` below 2010 — actuals feeds training targets and backtest
  ground truth; pre-2010 is career-history-only.
...
- `build_team_qb` sorts with a gsis_id tiebreak (2012 SF: Smith and
  ...
  run-to-run.
- StandardScaler's zero-variance fallback is load-bearing for `vs_adp` in
  fold 2012, whose training window is 2011 alone and 2011 has no ECR — zero
  train variance, coefficient fit as 0. That is now the ONLY affected fold
  (before the 2026-07-25 backfill, ECR started in 2015 and this held through
  fold 2015). Since 2011 left the training set on 2026-07-25 this fallback no
  longer fires anywhere: every tune and test fold now trains only on
  ECR-bearing seasons. Keep the fallback in place regardless.
- **`FIRST_TARGET = 2012` — 2011 is OUT and must stay out.** See "Three input
  defects" above for the full reasoning and the long comment in
  `bff/model.py`. Short version: 2011 has no ECR and never will, so its rows
  anchored on ADP with a fabricated `vs_adp = 0`, and this architecture learns
  a residual RELATIVE TO ITS ANCHOR. An earlier version of this bullet said
  the opposite and cited a −0.0047 cost; that was measured on the broken 2012
  ADP board and is void. Restoring 2011 bought +0.0153 of tune score
  (pre-07-27 measurement) from an input known to be wrong. And do NOT shift
  the whole protocol forward to buy a sixth ECR-clean fold: that moves 2018 —
  an ADP-loss season scored many times — out of the test set, manufacturing
  a better record by deletion.
- **`bff/actuals.py` position handling: never a bare `is_in(POS)` filter.**
  Rows whose nflverse position is null/FB/CB fall back to the player's
  ADP/ECR board position, and `check_board_coverage` asserts every ADP-board
  player with nonzero raw weekly points is present. Weakening either
  re-fabricates zero-point seasons for drafted players (see "The fourth
  input defect"). Investigate an assert failure, never relax it.
