"""RETIRED SOURCE — FantasyFootballCalculator PPR ADP. Reference only.

Was `bff/adp.py` until 2026-08-31, when the ADP source moved to FantasyPros
(`bff/fp_adp.py`) and this file was renamed. It is kept RUNNABLE, writing to a
NON-canonical path, for exactly one purpose: reproducing the FFC-era board so
old published numbers can be re-derived and compared. Nothing in the pipeline
reads its output.

Why it was replaced: FFC prunes its own stored boards and the damage is at the
TOP, not the tail. The 2022 half-PPR board is missing Austin Ekeler outright
(PPR ADP rank 3); 2022 loses 9 of the PPR top-120, 2025 loses 5. No FFC
endpoint has the missing players (checked: API at every league size, the
rendered page, and the CSV export), and half-PPR does not exist there before
2018. FantasyPros carries all of them, reaches 2012 in full PPR, and resolves
100% of the top 150 to gsis ids against FFC's 99.5%.

Output: data/processed/adp_ffc.parquet with columns
...
OUT = ROOT / "data" / "processed" / "adp_ffc.parquet"  # NOT canonical
